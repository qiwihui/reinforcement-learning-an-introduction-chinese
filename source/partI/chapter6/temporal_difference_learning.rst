...
构成GPI的两个过程之一驱动值函数以准确地预测当前策略的回报;这是预测问题。另一个过程推动政策在当地（例如，对于当前价值函数来说是“-greedy”）进行改进。当第一个过程基于经验时，出现了关于保持足够的探索的复杂性。我们可以根据TD控制方法进行分类。他们是否通过使用政策上或政策外的方法处理这种并发症.Sarsa是一种政策上的方法，而Q-learning是一种非政策性的方法。预期的Sarsa也是一种非政策性的方法，因为我们目前第三种方法可以扩展TD方法来控制我们在本章中没有包含的方法，称为演员 - 评论方法。这些方法在第13章中有详细介绍。

本章介绍的方法是目前使用最广泛的强化学习方法。这可能是由于它们非常简单：它们可以在线应用，只需极少的计算量，就可以通过与环境的交互来体验;它们可以通过可以用小型计算机程序实现的单个方程几乎完全表达。在接下来的几章中，我们扩展了这些算法，使它们更复杂，功能更强大。所有新算法将保留这里介绍的算法的本质：它们将能够在线处理经验，计算相对较少，并且它们将由TD错误驱动。本章介绍的TD方法的特殊情况应正确地称为一步法，表格式，无模型TD方法。在接下来的两章中，我们将它们扩展为n步形式（蒙特卡罗方法的链接）和包含环境模型的形式（计划和动态编程的链接）。然后，在本书的第二部分，我们将它们扩展到各种形式的函数逼近而不是表格（深度学习和人工神经网络的链接）。
...
正如我们在第1章中所概述的那样，TD学习的概念早期源于动物学习心理学和人工智能，最着名的是Samuel（1959）和Klopf（1972）的工作。 Samuel的工作被描述为第16.2节中的案例研究。 与道明学习有关的还有荷兰（1975,1976）早期关于价值预测一致性的想法。 这些影响了作者之一（Barto），他是1970年至1975年在荷兰教授的密歇根大学的研究生。 荷兰的想法引发了许多TD相关系统，包括Booker（1982）和荷兰斗式旅（1986）的工作，这与Sarsa有关，如下所述。

6.1-2

这些部分的大部分具体材料来自Sutton（1988），包括TD（0）算法，随机游走示例和术语“时间差异学习”。与动态亲的关系的表征  -  Watkins（1989），Werbos（1987）和其他人的文法和蒙特卡罗方法受到影响。 备份图的使用对于本书的第一版来说是新的。
基于Watkins和Dayan（1992）的工作，表格TD（0）被Sutton（1988）和大雁（1992）的概率1收敛。 Jaakkola，Jordan和Singh（1994）以及Tsitsiklis（1994）通过使用强大的现有随机近似理论的扩展，扩展和加强了这些结果。 其他扩展和概括将在后面的章节中介绍。

6.3

Sutton（1988）建立了批量训练下TD算法的最优性。照亮这一结果的是Barnard（1993）推导的TD算法，它是用于学习马尔可夫链模型的增量方法的一个步骤和用于从模型计算预测的方法的一个步骤的组合。术语确定性等价来自自适应对照文献（例如，Goodwin和Sin，1984）。

6.4

Sarsa算法由Rummery和Niranjan（1994）引入。他们结合人工神经网络对其进行了探索，称之为“改性连接主义Q学习”。萨尔顿（Sutton，1996）引入了“萨尔萨”这个名字。 Singh，Jaakkola，Littman和Szepesv ari（2000）证明了一步表格Sarsa（本章所述形式）的收敛性。 Tom Kalt提出了“多风的网格世界”的例子。
Holland（1986）斗式旅的想法演变成与Sarsa密切相关的算法。铲斗旅的最初想法涉及相互触发的规则链;它专注于将信用从当前规则传递回触发它的规则。随着时间的推移，斗式旅更像是TD学习，将信用归还给任何暂时的先前规则，而不仅仅是触发当前规则的规则。铲斗旅的现代形式，以各种自然方式简化，几乎与一步法Sarsa相同，如Wilson（1994）所详述。

6.5沃特金斯（1989）引入了Q学习，Watkins和Dayan（1992）对收敛证明的概述进行了严格的描述。 Jaakkola，Jordan和Singh（1994）以及Tsitsiklis（1994）证明了更一般的收敛性结果。

6.6预期的Sarsa算法由George John（1994）引入，他称之为“Q-learning”，并强调其优于Q-learning作为一种非策略算法。当我们在本书的第一版中将预期的Sarsa作为练习，或者对于van Seijen，van Hasselt，Whiteson和Weiring（2009），他们建立了预期的Sarsa的收敛性和条件时，John的工作并不为我们所知。将胜过常规的Sarsa和Q-learning。我们的图6.3是根据他们的结果改编的。 Van Seijen等人。将“预期的Sarsa”定义为一种独立的政策方法（正如我们在第一版中所做的那样），而现在我们将此名称用于目标和行为政策可能不同的通用算​​法。 van Hasselt（2011）注意到了预期的Sarsa的一般政策观点，他称之为“一般Q学习”。

6.7最大化偏见和双重学习被van Hasselt引入并进行了广泛的研究（2010年，2011年）。图6.5中的示例MDP改编自图4.1中的示例（van Hasselt，2011）。

6.8退休后的概念与“决定后的国家”的概念相同（Van Roy，Bertsekas，Lee和Tsitsiklis，1997; Powell，2011）。
